...
aov_count <- aov(OAKcount~Disturbance+Error(Unit),data=df)
summary(aov_count)
...
aov_drc23 <- aov(DRC_mm~Disturbance+Error(Unit/Plot_unique),data=ms23x)
summary(aov_drc23)
...
aov_xg24 <- aov(Extension_growth_cm~Disturbance+Error(Unit/Plot_unique),data=ms24x)
summary(aov_xg24)

aov_leaves24 <- aov(nleaves~Disturbance+Error(Unit/Plot_unique),data=ms24x)
...
#sdlg count
library(glmmTMB)
m_count <- glmmTMB(OAKcount~Disturbance+(1|Pair/Unit),
                ziformula=~1,family=nbinom2,data=df)
summary(m_count)

library(emmeans)
emm <- emmeans(m_count,pairwise~Disturbance,type="response")
emm
plot(emm)

#drc 2023 HAVE TO FIGURE OUT THE CORRECT ERROR TERM
library(lme4)
ms23x$logDRC <- log(ms23x$DRC_mm)
m_drc <- lmer(logDRC ~ Disturbance+(1|Pair/Plot_unique),data=ms23x)
summary(m_drc)
